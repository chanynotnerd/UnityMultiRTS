...
public class MouseDrag : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            start = Input.mousePosition;    // 마우스 위치정보 저장
            dragRect = new Rect();  // 메모리 할당해놓음
        }

        if (Input.GetMouseButton(0))
        {
            end = Input.mousePosition;  // 버튼을 누르고 있을때 end 변수에 현재 마우스 위치정보 저장

            // 마우스를 클릭한 상태로 드래그 하는 동안(start부터 end까지) 드래그 범위를 이미지로 표현
            DrawDragRectangle();
        }
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            CalculateDragRect();    // 화면상 드래그 범위 구함
            SelectUnits();  // 드래그 범위 포함된 유닛 선택 처리
